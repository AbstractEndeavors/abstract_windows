...
def get_proc_exe(pid: Union[str, int]) -> Optional[str]:
    return readlink_safe(f"/proc/{pid}/exe")

def get_proc_cwd(pid: Union[str, int]) -> Optional[str]:
    return readlink_safe(f"/proc/{pid}/cwd")
...
# -------------------------
def wmctrl_snapshot() -> List[Dict[str, str]]:
    """
...
def get_windows_list() -> List[str]:
    """Kept for compatibility; prefer wmctrl_snapshot() instead."""
